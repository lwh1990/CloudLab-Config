...
main(int argc, char **argv)
{
   ...
   for (lon = 0; lon < NLON; lon++)
    ...
    temp_out = (float*)malloc(NLAT*NLON*sizeof(float));
   ...
   if ((retval = ncmpi_put_var_float_all(ncid, lat_varid, &lats[0])))
      ERR(retval);
   if ((retval = ncmpi_put_var_float_all(ncid, lon_varid, &lons[0])))
      ERR(retval);
   ...
   {
      //start[0] = rank*(NREC/nprocs)+rec;
      start[0] = rec;

      if(rec%(NREC/100) == 0) printf("processed %d%%\n",rec/(NREC/100));
      for (lat = 0; lat < NLAT*NLON; lat++)
	 {
         int a = rand() % 10;
	    temp_out[lat] = a * 0.666;
	 }

    if ((retval = ncmpi_put_vara_float_all(ncid, temp_varid, start, count, 
				      temp_out)))
	 ERR(retval);
   }
   t2 = MPI_Wtime();
